...
public class DefaultMailSender implements MailSender {
	...
	public synchronized void send(Mail mail) {
		try {
			...
			Receiver receiver = mail.getReceiver();
			if (StringUtils.isEmpty(receiver.getNickname())) {
				sender.setNickname(sender.getAccount() + "-" + format(CloudUtils.localhosts()));
			}
			if (StringUtils.isEmpty(sender.getNickname())) {
				sender.setNickname(sender.getAccount());
			}
			// 1. 创建参数配置, 用于连接邮件服务器的参数配置
			Properties props = new Properties(); // 参数配置
			props.setProperty("mail.transport.protocol", "smtp"); // 使用的协议（JavaMail规范要求）
			props.setProperty("mail.smtp.host", sender.getSmtphost()); // 发件人的邮箱的 SMTP 服务器地址
			...
			// 2. From: 发件人
			message.setFrom(new InternetAddress(sender.getAccount(), sender.getNickname(), "UTF-8"));
			// 3. To: 收件人（可以增加多个收件人、抄送、密送）
			String address = receiver.getAddress();
			String[] splits = address.split(";");
			Address[] addresses = new Address[splits.length];
			for (int i = 0; i < splits.length; i++) {
				addresses[i] = new InternetAddress(splits[i], receiver.getNickname(), "UTF-8");
			}
			...
			try {
				...
				//
				transport.connect(sender.getAccount(), sender.getPassword());
	...
	private String format(List<String> localhosts) {
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < localhosts.size(); i++) {
			sb.append(localhosts.get(i));
			if (i < localhosts.size() - 1) {
				sb.append("-");
			}
		}
		return sb.toString();
